...
export default function LoginPage() {
  ...
  const handleLogin = () => {
    ...
    localStorage.setItem("shopUser", JSON.stringify({ name: user.name, email: user.email }));
    ...
    <main className="mx-auto min-h-[60vh] max-w-md px-6 py-12">
      <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
        ...
        <p className="mt-4 text-sm text-gray-600">
          New user?{" "}
          <Link href={`/register?redirect=${encodeURIComponent(redirectTo)}`} className="text-blue-600">
            Create account
          </Link>
        </p>
      </div>
